    ...
    */
    ...
    if(isset($action)) {
        ...
        if($action == "ladd" && checkParams([$lshort, $lname])) {
            ...
            $sql = 'UPDATE `content` SET `title`="'.$ctitle.'", `text`="'.htmlspecialchars($ctext).'", `image`="'.$file.'" WHERE id='.$cid;
        } elseif ($action == "cadd") {
            $sql = 'INSERT INTO `content` (`menu`,`title`,`text`,`image`) VALUES ('.$mid.', "'.$ctitle.'", "'.htmlspecialchars($ctext).'", "'.$file.'")';
        } elseif ($action == "idel") {
            $sql = 'UPDATE `content` SET `image`="" WHERE `id`='.$cid;
        }
                        ...
                        <div id="thumb">
                            <?php
                                if($row["image"] != "") {
                                    ...
                                    echo '<a href="?action=idel&lid='.$lid.'&cid='.$row["id"].'"><i class="fas fa-times-circle"></i></a>';
                                }
                            ?>
